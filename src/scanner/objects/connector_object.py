...
class ConnectorObject():
    ...
    def __init__(self, *args) -> None:
        if len(args) == 3:
            self.constructor_1(args[0], args[1], args[2])
        elif len(args) == 4:
            self.constructor_2(args[0], args[1], args[2], args[3])
        else:
            raise Exception("Wrong PssmObject constructor arguments.")

    def constructor_1(self, max_seq_length, mu=None, sigma=None):
    ...
    def constructor_2(self, config, max_seq_length, mu=None, sigma=None):
        """Connector constructor: 
            - gets/sets mu and sigma
            - sets all connector-specific configuration items
